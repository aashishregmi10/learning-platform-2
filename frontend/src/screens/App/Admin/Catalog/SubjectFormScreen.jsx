...
const SubjectFormScreen = () => {
  ...
  // A year only lists semesters when its program runs on a semester basis, so
  // the picker appears exactly for the subjects that need one.
  const semesterOptions = useMemo(() => {
    const y = years.find((x) => x._id === form.year) ?? existing?.data?.year;
    return y?.semesters ?? [];
  }, [years, form.year, existing]);
  ...
  const submit = async (e) => {
    ...
    const body = {
      ...
      ...(semesterOptions.length ? { semester: Number(form.semester) } : {}),
      ...
      <BreadcrumbLayout.Paper>
        <Box component="form" onSubmit={submit} sx={{ p: 3, display: "grid", gap: 3, gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" } }}>
          ...
          {semesterOptions.length > 0 && (
            <TextField select required size="small" label="Semester" value={form.semester} onChange={set("semester")}>
              {semesterOptions.map((s) => (
                <MenuItem key={s.semesterNumber} value={s.semesterNumber}>{s.name}</MenuItem>
              ))}
            </TextField>
          )}
          ...
          <Box sx={{ gridColumn: "1 / -1", display: "flex", alignItems: "center", gap: 2 }}>
            {form.thumbnail && (
              <Box component="img" src={form.thumbnail} alt="Cover preview" sx={{ width: 80, height: 60, objectFit: "cover", borderRadius: 1, border: `1px solid ${tokens.border}` }} />
            )}
            ...
            {!form.thumbnail && (
              <Typography variant="caption">Optional — falls back to a default illustration</Typography>
            )}
          </Box>
