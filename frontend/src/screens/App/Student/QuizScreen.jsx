...
const QuizScreen = () => {
  ...
  if (error) return <div style={{ padding: 24, color: "var(--status-danger-fg)" }}>{error?.data?.message || "This quiz isn't available."}</div>;
  if (!quiz) return null;
  ...
  return (
    <div style={{ maxWidth: 700, margin: "0 auto" }}>
      <Link to="/app/student" style={{ color: "#171717", fontSize: 14 }}>← Back to catalog</Link>
      <h1 style={{ color: "#171717", marginTop: 8, marginBottom: 20 }}>{quiz.title}</h1>
      {quiz.description && <p style={{ color: "#71717A", marginTop: -12, marginBottom: 20 }}>{quiz.description}</p>}
